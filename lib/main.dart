...
class AppState extends ChangeNotifier with WidgetsBindingObserver {
  ...
  void _syncBleKeepAlive() {
    ...
    if (!shouldRun) {
      _stopBleKeepAlive(
        reason:
            "connected=$isConnected lifecycle=$_appLifecycleState seenRx=${_bleManager.hasSeenProtocolRx} initDone=$_bleInitDoneForLink",
      );
      return;
    }
  ...
  bool _hasFreshBleRx([Duration window = const Duration(seconds: 4)]) {
    return _bleManager.hasRecentRx(window);
  }

  void _preferWriteWithResponse({required String reason}) {
  ...
  Future<bool> _waitForBleRx({
    ...
    final ok = _hasFreshBleRx();
    if (!ok) {
      _bleManager.log("RX WAIT ${phaseLabel}timeout");
    }
    return ok;
  }
  ...
  Future<bool> _ensureBleResponsive({
    ...
    if (recoverOk) {
      _bleManager.log("RX PREFLIGHT ${phaseLabel}recover-ok");
    }
    return recoverOk;
  }
  ...
  Future<bool> iniciarCiclo(String id) async {
    ...
    if (isConnected) {
      ...
      try {
        ...
        _preferWriteWithResponse(reason: "catalogo-before-primary-sequence");
        ...
        if (_bleManager.canObserveRx) {
          ...
          if (!catalogRxAfterRetry && isConnected && _bleManager.isConnected) {
            ...
            _preferWriteWithResponse(reason: "catalogo-before-hardwake");
