...
class WellnessTreatment {
  ...
  List<String> safetyNotes(bool isSpanish) =>
      isSpanish ? safetyNotesEs : _englishListOrFallback(safetyNotesEn, safetyNotesEs);
  ...
  String? originNote(bool isSpanish) => isSpanish
      ? originNoteEs
      : _englishOptionalOrFallback(originNoteEn, originNoteEs);
...
String? _englishOptionalOrFallback(String? english, String? spanish) {
  if ((english == null || english.trim().isEmpty) &&
      (spanish == null || spanish.trim().isEmpty)) {
    return null;
  }
  return _englishTextOrFallback(english ?? '', spanish ?? '');
}
...
String _runtimeTranslateEsToEn(String value, {bool titleLike = false}) {
  ...
  final replacements = <MapEntry<Pattern, String>>[
    ...
    const MapEntry('general wellness guidance for', 'General wellness guidance for'),
